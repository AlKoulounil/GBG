...
* - MultiValue return (quite slow, produce extra garbage each call)
	* 
	* * * * * * * * * * * * * */
namespace Tools.ExpressionParser
{
	...
	public class ExpressionParser
	{
		...
		private static Regex mComparatorParser = null;

		private static Regex ComparatorParser {
			get {
				if (mComparatorParser == null) {
					mComparatorParser = new Regex ("^([^=<>]*)([=<>]=?)([^=<>]*)$");
				}
				return mComparatorParser;
			}
		}

		public bool EvaluateBoolean (string aExpression)
		{
			if (ComparatorParser.IsMatch (aExpression)) {

				Match m = ComparatorParser.Match (aExpression);
				double firstValue = this.Evaluate (m.Groups[1].Value);
				double secondValue = this.Evaluate (m.Groups[3].Value);
				string comparator = m.Groups[2].Value;
				...
				switch (comparator) {
					...
					case ">=":
						return (firstValue >= secondValue);
					default :
						throw new Exception ("unrecognized comparator " + comparator + " in expression " + aExpression);
				}
			} else {
				throw new Exception ("there is no comparator (=, <, >, <= or >= in expression " + aExpression);
			}
			//TODO : contribute to the creator of this module to add EvaluateBoolean function, after well tested
		}
	}
}
